...
SparkleFormation.new('couchbase').load(:precise_ruby223_ami, :ssh_key_pair, :chef_validator_key_bucket, :git_rev_outputs).overrides do
...
EOF
  ...
  dynamic!(:launch_config_chef_bootstrap, 'couchbase', :instance_type => 'm3.medium', :security_groups => lookup.get_security_group_ids(vpc), :chef_run_list => ENV['run_list'])

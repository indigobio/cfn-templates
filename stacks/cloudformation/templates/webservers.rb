...
SparkleFormation.new('webserver').load(:precise_ami, :ssh_key_pair, :chef_validator_key_bucket).overrides do
...
EOF
  ...
  dynamic!(:launch_config_chef_bootstrap, 'webserver', :instance_type => 'm3.medium', :create_ebs_volumes => false, :security_groups => lookup.get_security_groups(vpc), :chef_run_list => 'role[base],role[webserver]')
  ...
  dynamic!(:launch_config_chef_bootstrap, 'customreports', :instance_type => 't2.small', :create_ebs_volumes => false, :security_groups => lookup.get_security_groups(vpc), :chef_run_list => 'role[base],role[custom_reports]')

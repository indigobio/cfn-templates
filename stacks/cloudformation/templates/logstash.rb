...
SparkleFormation.new('logstash').load(:precise_ruby223_ami, :ssh_key_pair, :chef_validator_key_bucket).overrides do
...
EOF
  ...
  dynamic!(:launch_config_chef_bootstrap, 'logstash', :instance_type => 'm3.large', :create_ebs_volumes => true, :volume_count => 2, :volume_size => 25, :security_groups => lookup.get_security_group_ids(vpc), :chef_run_list => ENV['run_list'])

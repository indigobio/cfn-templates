...
EOF
  ...
  dynamic!(:launch_config_chef_bootstrap, 'sitemanager', :instance_type => 't2.small', :security_groups => sgs, :chef_run_list => 'role[base],role[site_manager]')
  dynamic!(:auto_scaling_group, 'sitemanager', :launch_config => :sitemanager_launch_config, :subnets => subnets, :notification_topic => topic)

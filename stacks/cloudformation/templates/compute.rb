...
SparkleFormation.new('compute').load(:precise_ruby223_ami, :ssh_key_pair, :chef_validator_key_bucket).overrides do
...
EOF
  ...
  dynamic!(:launch_config_chef_bootstrap, 'compute', :instance_type => 'm3.medium', :create_ebs_volumes => false, :security_groups => lookup.get_security_group_ids(vpc), :chef_run_list => ENV['run_list'])
